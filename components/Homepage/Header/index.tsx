...
function HeaderComponent({ backgroundVideo }: IHeaderComponent) {
  ...
  const handleScroll = (mappedIntent: IMappedIntent) => {
    if (mappedIntent === "DOWN") scrollTo({ id: "0-slide" });
  };

  useEffect(() => {
    scrollSwipe({ ref: headerRef, handleScroll });
  }, []);

  return (
    <Cover id="home-header" ref={headerRef}>
      <Container>
        <Div>
          <VideoBlock>
