...
function getIndustries(labels: IUrl[], linkClickHandler: () => void) {
  return labels.map((label, index) => (
    <Industry key={index} onClick={linkClickHandler}>
      <Link href={`/expertise/industries/${label.url}`}>{label.title}</Link>
    </Industry>
  ));
}

function getTechnologies(
  labels: ITechnologiesLabels[],
  titleColor: string,
...
const ExpertiseHoverElements = forwardRef(
  ({ titleColor, isShow, backgroundColor }: IExpertiseHoverElements, ref) => {
      ...
      <MainWrapper titleColor={titleColor}>
        ...
        >
          ...
          <PositionLoader titleColor={titleColor}>
            <Loader width={40} heigh={40} viewBox="0 0 54 109" />
          </PositionLoader>
