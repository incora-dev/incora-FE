...
function arrowWithText(
  ...
  return (
    <ArrowWithTextBlock>
      <Arrow width={16} height={8} viewBox="0 0 26 14" />

      <H5 onClick={linkClickHandler}>
        <Link href={`/services/${url}`}>{text}</Link>
      </H5>
    </ArrowWithTextBlock>
  );
}

const useGetElementsTop = (
  titleColor: string,
  linkClickHandler: () => void
) => {
    ...
    <>
      <IconBlock>
        <TitleBlock titleColor={titleColor}>
          <TeamExtension width={41.14} height={35} viewBox="0 0 304 280" />

          <H4 onClick={linkClickHandler}>
            <Link href={`/services/team-extension/3`}>Team Extension</Link>
          </H4>
        </TitleBlock>
        {!isMobile && !isTablet && !isSmallTablet && (
          <Text>
            Alongside the core functional team, the acquisition of additional
            human resources would cover up the needed gaps and accelerate the
      ...
      <IconBlock>
        <TitleBlock titleColor={titleColor}>
          <DedicatedTeam width={41.14} height={35} viewBox="0 0 335 292" />

          <H4 onClick={linkClickHandler}>
            <Link href={`/services/dedicated-team/6`}>Dedicated Team</Link>
          </H4>
        </TitleBlock>
        {!isMobile && !isTablet && !isSmallTablet && (
          <Text>
...
function useGetElementsBottom(
  ...
  return (
    <>
      ...
      {!isMobile && !isTablet && !isSmallTablet && (
        <H6>
          Software development is aimed to provide you with support on each
          stage required for the successful launch of the product: from
...
function getServices(labels: IServiceUrl[], linkClickHandler: () => void) {
  return labels.map((label, index) => (
    <Service key={index} onClick={linkClickHandler}>
      <Link href={`/services/${label.url}`}>{label.title}</Link>
    </Service>
  ));
}
...
const ServicesHoverElement = forwardRef(
  ({ titleColor, isShow, backgroundColor }: IServicesHoverElement, ref) => {
      ...
      <MainWrapper titleColor={titleColor}>
        <Div ref={ref as any} backgroundColor={backgroundColor}>
          <Wrapper titleColor={titleColor}>
            <BlockWithIcons>
              <BlockWithIconsTop>{elementsTop}</BlockWithIconsTop>

              <BlockWithIconsBottom>{elementsBottom}</BlockWithIconsBottom>
            </BlockWithIcons>
          ...
          {!isMobile && !isTablet && !isSmallTablet && (
            <>
              <PositionLoader>
                <Loader width={169.22} height={169.22} viewBox="0 0 96 96" />
              </PositionLoader>

              <PositionLoader2 titleColor={titleColor}>
                <Loader2 width={98} height={98} viewBox="0 0 54 109" />
              </PositionLoader2>
